...
async function triggerWorkflow(){{
  ...
  }}catch(e){{ /* nieszkodliwe jeśli się nie uda - zadziała i tak w ciągu godziny */ }}
}}
...
async function submitOffer(){{
  ...
  try{{
    ...
    showStatus('Zapisano! Sprawdzanie ruszyło - odśwież tę stronę za ok. 1-2 minuty.', 'ok');
    cancelEdit();
  }}catch(e){{
...
async function deleteOffer(btn){{
  ...
  try{{
    ...
    await triggerWorkflow();
    showStatus('Usunięto! Strona zaktualizuje się za ok. 1-2 minuty.', 'ok');
    row.remove();
  }}catch(e){{
    showStatus('Błąd: ' + e.message, 'err');
  }}
